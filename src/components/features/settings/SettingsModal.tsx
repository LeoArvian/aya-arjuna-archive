...
import { X, Globe, AlertTriangle, Upload, Send, CheckCircle } from 'lucide-react';
import { useSettingsStore, type Language } from '../../../store/useSettingsStore';
import { motion, AnimatePresence } from 'framer-motion';
import { useTranslation } from 'react-i18next';
import ThemeSwitcher from './ThemeSwitcher';
import { reportsService } from '../../../services/reportsService'; // Import Service
import { toast } from 'sonner'; // Import Toast

export default function SettingsModal() {
  const { isSettingsOpen, closeSettings, language, setLanguage } = useSettingsStore();
  const [activeTab, setActiveTab] = useState<'lang' | 'report'>('lang');
        ...
        >
          ...
          <div className="flex border-b border-dark-border">
            <button 
              ...
              className={`flex-1 py-4 text-sm font-bold transition-colors flex items-center justify-center gap-2 ${activeTab === 'lang' ? 'bg-primary/10 text-primary border-b-2 border-primary' : 'text-white/50 hover:text-white hover:bg-white/5'}`}
            >
              <Globe size={16} /> Umum / Bahasa
            </button>
            <button 
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            {activeTab === 'lang' ? (
              <div className="space-y-8">
                 <LanguageSettings currentLang={language} setLang={setLanguage} />
...
function LanguageSettings({ currentLang, setLang }: { currentLang: Language, setLang: (l: Language) => void }) {
  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-white/50 text-xs font-bold uppercase mb-2">
        <Globe size={14} /> {t('settings.lang_desc')}
      </div>
      {languages.map((lang) => (
        <button
          key={lang.code}
          ...
          className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${
            currentLang === lang.code 
              ...
              : 'bg-black/20 border-white/10 text-white/70 hover:bg-white/5 hover:border-white/30'
          }`}
        >
          <span className="flex items-center gap-3 font-medium">
            <span className="text-2xl">{lang.flag}</span> {lang.label}
          </span>
          {currentLang === lang.code && <CheckCircle size={20} className="text-primary" />}
        </button>
      ))}
    </div>
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {category === 'other' && (
            <input 
              ...
              placeholder={t('settings.report.other_placeholder')}
              className="w-full mt-2 bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-primary outline-none animate-in fade-in slide-in-from-top-2"
            />
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <textarea 
          ...
          className="w-full h-24 bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-primary outline-none resize-none"
          placeholder={t('settings.report.desc_placeholder')}
        />
      </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <label 
            htmlFor="report-file"
            className="flex items-center justify-center gap-2 w-full p-4 border border-dashed border-white/20 rounded-lg cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all"
          >
      ...
      <div className="flex gap-3 pt-4 border-t border-white/10">
        ...
        <button 
          type="submit"
          disabled={sending}
          className="flex-1 py-3 bg-primary hover:bg-primary-dark text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
        >
          {sending ? 'Mengirim...' : <><Send size={18} /> {t('settings.report.submit')}</>}
        </button>
      </div>
    </form>
  );
